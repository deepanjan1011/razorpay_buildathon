...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/diagrams/doors-dark.svg">
  <img alt="Browsing the product feed is unauthenticated by design. Creating and completing a checkout session require an agent credential, and completing additionally requires a signed mandate. The webhook verifies an HMAC over the raw bytes before parsing." src="docs/diagrams/doors-light.svg">
</picture>
...
Four of these are worth a judge's time, and they are not the same document.

| | |
|---|---|
| [`docs/OBSTACLES.md`](docs/OBSTACLES.md) | **Start here if you read one.** Every defect, dated: what broke, what it cost, what found it. Includes the ones that were embarrassing — a price parser that returned ₹0.13, a withholding rule that emptied the feed, an invariant that was false in production while 32 tests agreed with it. |
| [`docs/DESIGN.md`](docs/DESIGN.md) | Phase 0, written before any code and corrected against the spec rather than reconciled afterwards. §2 is the payment-handler reasoning, §3 the mandate model and the two-clocks decision, §5 the failure path. |
| [`docs/PLAN.md`](docs/PLAN.md) | The Phase 1 plan — the normalized schema, the taxonomy, and §4's catalogue of spreadsheet mess with a fixture for each case. §8 is the gate list, including the one box still open. |
| [`docs/NORMALIZATION-EVAL.md`](docs/NORMALIZATION-EVAL.md) | Generated by `npm run eval`, never hand-edited. Carries the accuracy number, its prompt fingerprint, every failure, and the fields it says it cannot measure. |
| [`docs/TRANSCRIBING.md`](docs/TRANSCRIBING.md) | How the real sheet was transcribed and labelled, and the permission it rests on. |
| [`docs/diagrams/`](docs/diagrams/) | The diagrams above, as standalone SVGs in light and dark. |

Two claims in this README are checkable rather than assertable:

- **the accuracy number** — `npm run eval` regenerates `NORMALIZATION-EVAL.md`
  from the labelled fixture, and the run history shows it moving 95% → 99% → 100%
- **ACP conformance** — `npm run conformance` validates what came back over HTTP
  against the pinned schemas, exits non-zero on failure, and reports every
  declared deviation as its own line rather than passing silently
