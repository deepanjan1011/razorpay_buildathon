# PLAN — Phase 1: catalog ingest, normalization, feed

*Renamed from `PHASE-1.md`. The filename was the only thing that changed; this
is still the Phase 1 plan specifically, and the later phases live in
`DESIGN.md` §8. Code comments cite it as `PLAN.md §N`.*
...
/docs
  DESIGN.md  PLAN.md  OBSTACLES.md  NORMALIZATION-EVAL.md
...
These were written before the work, and three of them stayed unticked long after
they were met — the boxes describe the gate, and nothing updates them but a
hand. An unticked box that is actually done reads as an unfinished project,
which is the opposite failure to the one this list exists to prevent.

- [x] A real merchant spreadsheet parses end to end — `fixtures/real-nellaikuttam-snacks.xlsx`,
      78 products transcribed from one Chennai seller's public IndiaMART listing
- [x] Every §4 mess case has a passing test
- [x] Flagged products are reviewable, not silently dropped or guessed —
      withheld *with a reason* by the projection, and `/catalogue` now shows
      every held row with the flags that blocked it, the sheet row it came from
      and the offending cell: *"Lemon Pickle, row 15, `150-200`,
      PRICE_AMBIGUOUS"*. A reason a merchant cannot locate is not actionable,
      which is what the provenance was carried the whole way for.
      **Still not built: editing one in the app.** The route back is fixing the
      sheet and uploading again, which resumes rather than re-spends. Reviewable
      is done; repairable is not, and the page says so rather than implying
      otherwise
- [x] `NORMALIZATION-EVAL.md` exists with a real, un-tuned number — 470/470
      field observations across 78 hand-labelled products. Generated by
      `npm run eval`, never hand-edited; the run history keeps 95% → 99% → 100%
      because a number that moved shows what the eval caught
...
- [x] Every variant traces to its source row — `Provenance` is required by the
      type and populated by the normalizer (`provenanceFor`, one per row):
      source file, sheet, row number, and the original cells
